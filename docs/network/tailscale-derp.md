...
graph TD
    User[外部用户 (手机/电脑)]
    NAS[家中 NAS]
    OfficialDERP[官方 DERP (海外)]
    CustomDERP[自建 DERP (阿里云/腾讯云)]

    User -- P2P 直连 (最佳) --> NAS
    User -. 打洞失败 .-> OfficialDERP
    OfficialDERP -. 延迟高/丢包 .-> NAS
    
    User == 强制走自建 DERP ==> CustomDERP
    CustomDERP == 低延迟/满速 ==> NAS

    style OfficialDERP fill:#f99,stroke:#333,stroke-dasharray: 5 5
    style CustomDERP fill:#9f9,stroke:#333,stroke-width:2px
```
